...
class ACBScraper:
    ...
    @staticmethod
    def _match_filename(match: MatchInfo) -> str:
        """
        Genera un nombre de fichero descriptivo para el partido:
          20260321_104658_SurneBilbao_vs_MoraBancAndorra
        - Fecha en formato YYYYMMDD (extraída del campo start ISO)
        - matchId
        - Nombres de equipos sin espacios ni caracteres especiales
        """
        import re
        # Fecha: "2026-03-21T18:00:00Z" → "20260321"
        try:
            date_str = match.date[:10].replace("-", "")   # "20260321"
        except Exception:
            date_str = "00000000"

        def clean(name: str) -> str:
            # Quitar tildes y caracteres especiales, reemplazar espacios por nada
            replacements = {
                "á":"a","é":"e","í":"i","ó":"o","ú":"u",
                "Á":"A","É":"E","Í":"I","Ó":"O","Ú":"U",
                "ñ":"n","Ñ":"N","ü":"u","Ü":"U",
            ...
            # Eliminar cualquier otro carácter que no sea alfanumérico
            name = re.sub(r"[^A-Za-z0-9]", "", name)
            return name

        home = clean(match.home_team_name)
        away = clean(match.away_team_name)
        return f"{date_str}_{match.match_id}_{home}_vs_{away}"
    ...
    def run(self, match_ids: list[int] | None = None):
        ...
        for match_id in match_ids:
            ...
            if match:
                ...
                total_plays += len(match.plays)

        log.info(f"✅ {len(match_ids)} partidos, {total_plays} eventos guardados en {self.output_dir}")

    def _save_match(self, match: MatchInfo):
        """Guarda un partido individual en CSV + JSON con nombre descriptivo."""
        base = self._match_filename(match)

        # ── CSV ──────────────────────────────────────────────────────────────
        if match.plays:
            csv_path = self.output_dir / f"{base}_plays.csv"
            fields   = [k for k in asdict(match.plays[0]).keys() if k != "raw"]
